...
check_load_pkgs <- function() {
        ...
        if(!require(ggplotify)) {
                message("installing the 'ggplotify' package")
...
nys <- data.table::fread("/home/ed/repos/COVID/nys_back")
```
...
ggplot(data=ny_map3) +
       ...
       geom_text_repel(data=filter(ny_map3, R>0, LabelDirection=="A"), 
                        mapping=aes(X, Y, label=County),
                        size=4) +
        geom_text_repel(data=filter(ny_map3, R>0, LabelDirection=="B"), 
                         mapping=aes(X, Y, label=County),
                         size=4) +
        ylim(40,46)+
