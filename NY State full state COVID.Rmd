...
United_States_map <- cbind(United_States_map,
                           st_coordinates(st_centroid(United_States_map$geometry))) %>%
     select("State" = NAME, X, Y, f, geometry)
...
```{r NYS Deaths}
ggplot(data = stateDeaths) +
     geom_point(mapping = aes(x = date, y = d)) +
     geom_label_repel(
          data = filter(stateDeaths, date == max(date)),
          aes(
               x = date,
               y = d,
               label = sprintf("%i", d)
...
```{r US Map R, fig.width= 20, fig.height=20}
ggplot(data = us_map) +
     geom_sf(aes(fill = R,
                 geometry=geometry)) +
     scale_fill_stepsn(
