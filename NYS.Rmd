...
check_load_pkgs <- function(packages) {
  ...
  for (pkg in packages) {
    ...
    load_status <- tryCatch({
      ...
      return(paste("Failed to load", package, "Error:", e))
    })
    ...
    if (inherits(load_status, "try-error")) {
      ...
      message(paste("Successfully loaded:", package))
    }
  }
}
...
# Function to estimate R for a single FIPS code
estimate_R_for_fips <- function(df, fips, time_window) {
  incid_data <- df %>% 
    filter(f == fips, !is.na(I)) %>% 
    pull(I)
  
  total_cases <- sum(incid_data)
  num_days <- length(incid_data)
  
  if (total_cases * 7 / num_days > 11) {
    t_start <- seq(2, num_days - time_window)
    ...
    conf_l <- R - 1.96 * std / sqrt(n)
    conf_h <- R + 1.96 * std / sqrt(n)
    return(data.frame(f = fips, R = R, Std = std, Confl = conf_l, Confh = conf_h))
  } else {
    return(data.frame(f = fips, R = NA, Std = NA, Confl = NA, Confh = NA))
  }
}

# Main function to estimate R for all FIPS codes
makeRoutput <- function(df, time_window) {
  lof <- df %>% 
    distinct(f) %>% 
    filter(!is.na(f)) %>% 
    pull(f)
  
  ROutput <- purrr::map_dfr(lof, ~ estimate_R_for_fips(df, ., time_window))
